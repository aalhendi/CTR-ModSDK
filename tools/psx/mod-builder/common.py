...
def find_sdk_root() -> pathlib.Path:
    env_root = os.environ.get("CTR_MODSDK_ROOT")
    candidates = []
    if env_root:
        candidates.append(pathlib.Path(env_root))
    candidates.extend(pathlib.Path.cwd().resolve().parents)
    candidates.insert(0, pathlib.Path.cwd().resolve())

    for candidate in candidates:
        if (candidate / CONFIG_FILE).exists() and (candidate / "disc.json").exists():
            return candidate

    raise FileNotFoundError("Could not find CTR-ModSDK root. Set CTR_MODSDK_ROOT or run from inside CTR-ModSDK.")

DIR_GAME = find_sdk_root()
CONFIG_PATH = DIR_GAME / CONFIG_FILE
logger.debug(f"CTR_MODSDK_ROOT: {DIR_GAME}")
